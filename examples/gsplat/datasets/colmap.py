...
class Dataset:
    ...
    def __init__(
        ...
        if self.parser.test_every == 1:
            ...
            json_path = "/efs/user_folders/nitzanh/dl3dv_fold_8_kmeans_input_idx.json"
            all_indices = np.arange(len(self.parser.image_names))
            
            # 2. Identify the current scene (extract leaf directory name)
            # e.g., "data/032dee9f..." -> "032dee9f..."
            scene_id =  Path(self.parser.data_dir).resolve().parents[0].name
            ...
            if os.path.exists(json_path):
                ...
                scene_entry = next((item for item in split_data if item["scene_name"] == scene_id), None)
                
                if scene_entry and "fold_8_kmeans_16_input" in scene_entry:
                    # 3. Apply the Best-8 strategy: stride the 16 k-means poses by 2
                    k16_list = scene_entry["fold_8_kmeans_16_input"]
                    train_indices = np.array(k16_list)#[::2]) # all 16
                    print(f"[{split.upper()}] Found JSON split for {scene_id}. Using {len(train_indices)} k-means views.")

            # 4. Final index assignment
            if train_indices is not None:
                if split == "train":
                    self.indices = train_indices
                else:
                    # Validation is everything NOT in the training set
                    self.indices = np.array([i for i in all_indices if i not in train_indices])
            else:
                raise
